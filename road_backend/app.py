...
# ==========================================================
    ...
    with tab_main:
        ...
        # ==========================================
            ...
            # --- ROW 2: DONUT INFOGRAPHICS ---
            col_d1, col_d2 = st.columns(2)
            
            with col_d1:
                st.markdown("#### 🚀 Drive Mode Distribution")
                # Pull metrics from SQLite DB
                ...
                fig_mode = go.Figure(data=[go.Pie(labels=labels_mode, values=vals_mode, hole=.4, 
                                                  marker_colors=['#00cc96', '#ab63fa', '#ff4b4b'])])
                fig_mode.update_layout(template='plotly_dark', plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', 
                                       margin=dict(t=20, b=20, l=20, r=20), height=350)
                st.plotly_chart(fig_mode, use_container_width=True)

            with col_d2:
                st.markdown("#### 🔋 Powertrain State (Regen vs. Coast vs. Push)")
                # Calculate active states from the DataFrame Torque column
                # Assumes data is recorded at 2Hz (0.5s per row)
                if 'Motor_Torque [Nm]' in df.columns:
                    ...
                    fig_pwr = go.Figure(data=[go.Pie(labels=labels_pwr, values=vals_pwr, hole=.4, 
                                                     marker_colors=['#ff4b4b', '#888888', '#00cc96'])])
                    fig_pwr.update_layout(template='plotly_dark', plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', 
                                          margin=dict(t=20, b=20, l=20, r=20), height=350)
                    st.plotly_chart(fig_pwr, use_container_width=True)
                else:
                    st.error("Motor_Torque [Nm] data missing from file. Cannot compute powertrain states.")

            st.divider()

            # --- ROW 3: DUAL-AXIS DERATION MAP ---
            st.markdown("### ⚠️ Motor Torque vs. Thermal Deration Map")
            st.write("Visualizing how Torque demands correlate with Motor Temperature, exposing torque cuts during thermal breaches.")
            
            if 'Motor_Torque [Nm]' in df.columns and 'Motor_Temp [C]' in df.columns:
                fig_derate = go.Figure()
                
                # Plot 1: Motor Torque (Filled Area on Primary Y-Axis)
                fig_derate.add_trace(go.Scatter(
                    x=df['Time'], y=df['Motor_Torque [Nm]'], 
                    mode='lines', fill='tozeroy', 
                    name='Motor Torque Demanded (Nm)', 
                    line=dict(color='#1f77b4', width=1), opacity=0.4
                ))
                
                # Plot 2: Motor Temp (Solid Line on Secondary Y-Axis)
                fig_derate.add_trace(go.Scatter(
                    x=df['Time'], y=df['Motor_Temp [C]'], 
                    mode='lines', name='Motor Temp (°C)', 
                    yaxis='y2', line=dict(color='#ff4b4b', width=3)
                ))
                
                # The Limit Line
                limit_temp = 125.0
                fig_derate.add_hline(y=limit_temp, line_dash="dash", line_color="gold", annotation_text="Safety Limit (125°C)", yref="y2")
                
                # Layout magic for Dual Axes
                fig_derate.update_layout(
                    template='plotly_dark', plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)',
                    hovermode="x unified", height=500, margin=dict(t=30, b=30, l=30, r=50),
                    xaxis=dict(title="Time (s)", showgrid=True, gridcolor='#333'),
                    yaxis=dict(title="Motor Torque (Nm)", showgrid=True, gridcolor='#333'),
                    yaxis2=dict(
                        title="Motor Temperature (°C)", 
                        overlaying='y', side='right', 
                        showgrid=False, range=[0, df['Motor_Temp [C]'].max() + 15]
                    ),
                    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
                )
                st.plotly_chart(fig_derate, use_container_width=True)
            else:
                st.error("Required columns (Motor_Torque or Motor_Temp) missing from this test log.")

    # --- CUMULATIVE RISE (ΔT) TAB (Only visible for Channel 1) ---
    if "Channel 1" in channel_view:
        with tab_delta:
            if not active_channels: st.info("👆 Please select at least one Thermal Channel.")
            else:
